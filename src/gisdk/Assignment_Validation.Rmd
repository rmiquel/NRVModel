...
#links <- readr::read_csv("C:/projects/NRV/repo/scenarios/Base_2016/outputs/summary/links_temp.csv")
...
links_vmt <- links %>%
  mutate(
    model_vmt = volume * Length,
    count_vmt = count * Length
  )
link_stats_table(
  links_vmt, "model_vmt", "count_vmt", group_field = "facility_group", type = "flow") %>%
  mutate_at(
    vars(`Observed Flow`, `Model Flow`),
    .funs = funs(format(round(., 0), big.mark = ","))
  ) %>%
  rename(`Observed VMT` = `Observed Flow`, `Model VMT` = `Model Flow`) %>%
  kable(digits = 2, caption = "Total VMT by Facility Group", align = "lrrr")
