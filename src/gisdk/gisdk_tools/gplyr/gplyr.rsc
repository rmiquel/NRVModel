...
Macro "test gplyr"
  ...
  if dir = null 
    then dir = ChooseDirectory("Select the 'unit_test_data' folder", )
  on escape default
  ...
  df = CreateObject("df")
  vars = GetObjectVariableNames(df)
  if vars.length <> 3 then Throw("test: empty df not created properly")

  // test creation with file paths and views
  df = CreateObject("df", csv_file)
  df = CreateObject("df", bin_file)
  view = OpenTable("table", "FFB", {bin_file,})
  df = CreateObject("df", view)
  CloseView(view)
  
  // test creation with named array
  array = null
  array.ID = {1, 2, 3}
  array.HH = {4, 5, 6}
  ...
  // df.mutate("bad2", {1, 2}) // should raise a length error
  ...
  */
  ...
  fields = "Count"
  stats = {"sum", "avg"}
  df.summarize(fields, stats, "true")
  answer1 = {140, 150, 110}
  answer2 = {70, 75, 55}
  ...
  if df.groups <> null then Throw("test: summarize() failed to remove groups")

  // test deprecated summarize
  df = CreateObject("df")
  df.read_csv(csv_file)
  df.group_by("Color")
  opts = null
  opts.Count = {"sum", "avg"}
  df = df.summarize_deprecated(opts, "false")
  answer1 = {140, 150, 110}
  answer2 = {70, 75, 55}
  for a = 1 to answer1.length do
    if df.tbl.sum_Count[a] <> answer1[a] then Throw("test: summarize_deprecated() failed")
    if df.tbl.avg_Count[a] <> answer2[a] then Throw("test: summarize_deprecated() failed")
  end
  if df.groups <> null then Throw("test: summarize_deprecated() failed to remove groups")
  ...
  df = CreateObject("df")
  df.read_csv(csv_file)
  result = df.get_row(2, {"Color", "Count"}, "false")
  answer = {"Yellow", 75}
  for a = 1 to answer.length do
    if answer[a] <> result[a] then Throw("test: get_row() failed")
  end
